...
async function verificarRateLimit(client: ReturnType<typeof db>, email: string): Promise<boolean> {
  ...
  if (!data) return true // primeira tentativa
  if (data.bloqueado_ate && new Date(data.bloqueado_ate) > new Date()) return false // bloqueado
  return true
}
...
async function registrarTentativa(client: ReturnType<typeof db>, email: string, sucesso: boolean) {
  if (sucesso) {
    ...
    await client.from('portal_auth_log').insert({ tabela: 'portal_admin_users', email, sucesso: true })
    return
  }
  const { data } = await client.from('portal_login_attempts')
  ...
  await client.from('portal_auth_log').insert({
    tabela: 'portal_admin_users', email, sucesso: false,
...
export async function POST(req: NextRequest) {
  ...
  const { data: user } = await client
    ...
    .select('id, nome, email, ativo, senha, senha_hash')
  ...
  const senhaOk = user.senha_hash === senhaHash || user.senha === senha
  if (!senhaOk) {
    await registrarTentativa(client, email, false)
    return NextResponse.json({ error: 'Credenciais inválidas' }, { status: 401 })
  }
  ...
  if (!user.senha_hash) {
    await client.from('portal_admin_users').update({ senha_hash: senhaHash }).eq('id', user.id)
  }
